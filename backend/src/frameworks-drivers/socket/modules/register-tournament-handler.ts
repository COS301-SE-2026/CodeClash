...
export function registerTournamentHandlers(io: Server, socket: Socket, deps: TournamentDeps) {
    ...
    registerHandler(socket, "host_tournament", (socket, data: { start_date: Date, match_mode: MatchMode, host: PlayerDTO, title: string, min_players: number}) =>
        hostTournament(io, socket, data.start_date, data.match_mode, data.host, data.title,data.min_players, deps.tournament_service));
