...
export function registerMatchmakingHndlers(io: Server, socket: Socket, deps: MatchmakingDeps) {
    ...
    socket.on('decline_match', (data: { group_id: string, match_mode: MatchMode }) => matchDeclined(io, data.group_id, data.match_mode, deps.matched_users_service, deps.matchmaking_service));
}