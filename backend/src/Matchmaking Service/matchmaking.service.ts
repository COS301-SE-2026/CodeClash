...
async function enqueue(user: UserDto, queue: string): Promise<boolean> {
    if (queue != "math" && queue != "programming") return false;
...
async function dequeue(user_id: number, queue: string): Promise<boolean> {
    ...
    const rem_joined_hash = await redis.hdel(`user:${user_id}`, "user_joined_at", "user_game_mode");
...
async function matchmaking(user: UserDto) {
    ...
    if (user.game_mode != "math" && user.game_mode != "programming")
    ...
    if (players.length == 0) {
        ...
        await redis.hdel(`user:${user.id}`, "user_joined_at", "user_game_mode");
        await redis.hdel(`user:${match.user_id}`, "user_joined_at", "user_game_mode");
...
async function prog_queue_length(): Promise<number> {
    return await redis.zcard('programming');
}
